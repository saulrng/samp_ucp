...
if($data->Rank == 6)
	return print_r(json_encode(array('message' => 'Player <i>'.$data->name.'</i> has maximum rank (6).','color' => 'red')));
...
if($qa)
	return print_r(json_encode(array('message' => 'Player <i>'.$data->name.'</i> is now rank '.$ranks->$rank.'.','color'=>'green','rank' => $ranks->$rank)));
else
	return print_r(json_encode(array('message' => 'An error occured.','color' => 'red')));
